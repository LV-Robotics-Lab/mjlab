...
_PM1_FALL_DATA_RESET_DIR = (
  Path(__file__).resolve().parents[5].parent / "data" / "amp_pm1_fall"
)
...
def pm1_flat_falling_env_cfg(
  ...
  *,
  ...
  enable_amp_env: If True, keep ``cfg.amp`` and load expert ``.npz`` for AMP.
    If False, set ``cfg.amp = None`` (no AMPHelper / no ``amp`` obs in wrapper).
  """
  del (
  ...
  if (
    not use_data_reset and cfg.curriculum is not None and "reset_init" in cfg.curriculum
  ):
  ...
  if enable_amp_env:
    assert cfg.amp is not None, (
      "make_fall_env_cfg must define cfg.amp when enable_amp_env=True"
    )
    cfg.amp.motion_file = [
