...
def _get_body_indexes(
  command: FallCommand, body_names: tuple[str, ...] | None
) -> list[int]:
  return [
    i
...
def fall_global_anchor_position_error_exp(
...
def fall_global_anchor_orientation_error_exp(
  env: ManagerBasedRlEnv, command_name: str, std: float
) -> torch.Tensor:
  command = cast(FallCommand, env.command_manager.get_term(command_name))
...
def fall_relative_body_position_error_exp(
...
def fall_relative_body_orientation_error_exp(
  env: ManagerBasedRlEnv,
  command_name: str,
  std: float,
  body_names: tuple[str, ...] | None = None,
...
def fall_global_body_linear_velocity_error_exp(
  env: ManagerBasedRlEnv,
  command_name: str,
  std: float,
  body_names: tuple[str, ...] | None = None,
...
def fall_global_body_angular_velocity_error_exp(
  env: ManagerBasedRlEnv,
  command_name: str,
  std: float,
  body_names: tuple[str, ...] | None = None,
...
def feet_relative_position_error_exp(
  ...
  """Reward fall feet position relative to anchor/torso.
...
def projected_gravity_fall_reward(
  env: ManagerBasedRlEnv, command_name: str, std: float
) -> torch.Tensor:
  """Reward for fall projected gravity vector.
...
def ankle_pitch_joint_fall_reward(
  env: ManagerBasedRlEnv,
  command_name: str,
  std: float,
...
def ankle_roll_joint_fall_reward(
  env: ManagerBasedRlEnv,
  command_name: str,
  std: float,
  asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
...
#     command_name: Name of the fall command (for getting foot indices if needed)
