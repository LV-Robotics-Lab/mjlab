...
class FallLoader:
  def __init__(
    self, fall_file: str, body_indexes: torch.Tensor, device: str = "cpu"
  ) -> None:
    data = np.load(fall_file)
...
class FallCommand(CommandTerm):
  cfg: FallCommandCfg
  _env: ManagerBasedRlEnv

  def __init__(self, cfg: FallCommandCfg, env: ManagerBasedRlEnv):
    ...
    self.fall_anchor_body_index = self.cfg.body_names.index(self.cfg.anchor_body_name)
    ...
    self.fall = FallLoader(
      self.cfg.fall_file, self.body_indexes, device=self.device
    )
    ...
    self.bin_count = int(self.fall.time_step_total // (1 / env.step_dt)) + 1
  ...
  def future_frames_command(self) -> torch.Tensor:
    ...
    # 处理边界情况：限制在有效范围内
    max_valid_index = self.fall.time_step_total - 1
    future_indices = torch.clamp(future_indices, 0, max_valid_index)
    
    # 获取未来9帧的关节位置和速度
    future_pos_frames = self.fall.joint_pos[future_indices]  # (N, 9, num_joints)
    future_vel_frames = self.fall.joint_vel[future_indices]  # (N, 9, num_joints)
  ...
  def joint_pos(self) -> torch.Tensor:
    return self.fall.joint_pos[self.time_steps]

  @property
  def joint_vel(self) -> torch.Tensor:
    return self.fall.joint_vel[self.time_steps]

  @property
  def body_pos_w(self) -> torch.Tensor:
    return (
      self.fall.body_pos_w[self.time_steps] + self._env.scene.env_origins[:, None, :]
    )

  @property
  def body_quat_w(self) -> torch.Tensor:
    return self.fall.body_quat_w[self.time_steps]

  @property
  def body_lin_vel_w(self) -> torch.Tensor:
    return self.fall.body_lin_vel_w[self.time_steps]

  @property
  def body_ang_vel_w(self) -> torch.Tensor:
    return self.fall.body_ang_vel_w[self.time_steps]

  @property
  def anchor_pos_w(self) -> torch.Tensor:
    return (
      self.fall.body_pos_w[self.time_steps, self.fall_anchor_body_index]
      + self._env.scene.env_origins
    )

  @property
  def anchor_quat_w(self) -> torch.Tensor:
    return self.fall.body_quat_w[self.time_steps, self.fall_anchor_body_index]

  @property
  def anchor_lin_vel_w(self) -> torch.Tensor:
    return self.fall.body_lin_vel_w[self.time_steps, self.fall_anchor_body_index]

  @property
  def anchor_ang_vel_w(self) -> torch.Tensor:
    return self.fall.body_ang_vel_w[self.time_steps, self.fall_anchor_body_index]
  ...
  def _adaptive_sampling(self, env_ids: torch.Tensor):
    ...
    if torch.any(episode_failed):
      current_bin_index = torch.clamp(
        (self.time_steps * self.bin_count) // max(self.fall.time_step_total, 1),
        0,
        self.bin_count - 1,
      )
  ...
  def _uniform_sampling(self, env_ids: torch.Tensor):
    self.time_steps[env_ids] = torch.randint(
      0, self.fall.time_step_total, (len(env_ids),), device=self.device
    )
  ...
  def _update_command(self):
    self.time_steps += 1
    env_ids = torch.where(self.time_steps >= self.fall.time_step_total)[0]
    if env_ids.numel() > 0:
...
class FallCommandCfg(CommandTermCfg):
  fall_file: str
  anchor_body_name: str
  body_names: tuple[str, ...]
  asset_name: str
  class_type: type[CommandTerm] = FallCommand
