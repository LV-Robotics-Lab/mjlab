...
# Cached protector map: (y_grid, z_grid, values_2d, dy, dz) per path；与 1215698 一致，不缓存/读取 TSV 密度
...
def _load_yz_protector_tsv(
  ...
  与 1215698 一致：仅返回网格与间距，不解析 TSV 中的 # density；护具密度由 reward 参数 density_protector 提供。
  Returns:
...
def reduce_contact_force_weighted(
  ...
  Args:
    ...
    density_protector: 护具密度，用于力衰减公式（与 1215698 一致，仅用此参数，不读 TSV）。
  ...
  if protector_map_dir is not None:
    ...
    F_after_kN = _force_after_protector_torch(
      F_before_kN, t_mm, density_protector, C, alpha_f, beta, gamma
    )
