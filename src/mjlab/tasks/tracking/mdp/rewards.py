...
def _get_joint_indexes(asset: Entity, joint_patterns: tuple[str, ...]) -> list[int]:
  """根据模式查找关节索引。
...
def reduce_contact_force_weighted(
  ...
  average_term = weighted_force_norm.sum(dim=-1) / num_active_contacts.squeeze(
    -1
  )  # [B]
...
def ankle_pitch_joint_tracking_reward(
  ...
  # 查找左右脚踝 pitch 关节索引
  pitch_indices = _get_joint_indexes(asset, ("J04_ANKLE_PITCH_L", "J10_ANKLE_PITCH_R"))
  if not pitch_indices:
    device = getattr(command, "device", None) or torch.device(
...
def ankle_roll_joint_tracking_reward(
  ...
  asset: Entity = env.scene[asset_cfg.name]
  # 查找左右脚踝 roll 关节索引
  roll_indices = _get_joint_indexes(asset, ("J05_ANKLE_ROLL_L", "J11_ANKLE_ROLL_R"))
  if not roll_indices:
    device = getattr(command, "device", None) or torch.device(
      "cuda" if torch.cuda.is_available() else "cpu"
    )
...
def ankle_joint_smoothness_penalty(
  ...
  ankle_indices = _get_joint_indexes(
    asset,
    ("J04_ANKLE_PITCH_L", "J05_ANKLE_ROLL_L", "J10_ANKLE_PITCH_R", "J11_ANKLE_ROLL_R"),
  )
