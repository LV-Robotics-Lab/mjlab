...
def history_observations(
  ...
  if reset_mask.any():
    env._obs_history[reset_mask] = (
      current_obs[reset_mask].unsqueeze(1).repeat(1, history_steps, 1)
    )
...
def base_height_diff_10frames(
  ...
  future_steps = torch.arange(0, 10, device=current_time.device).unsqueeze(
    0
  )  # [1, 5] (0,1,2,3,4)
  ...
  ref_body_pos_w = command.motion.body_pos_w[
    frame_indices, command.motion_anchor_body_index
  ] + command._env.scene.env_origins.unsqueeze(1)  # [N, 5, 3]
