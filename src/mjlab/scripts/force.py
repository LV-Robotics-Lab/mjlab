...
def _plot_contact_curves(
  ...
  for ax, (group_key, title) in zip(axes, ordered_groups, strict=True):
    ...
    for body_name, (time_array, force_array) in body_data.items():
      if len(time_array) > 0 and len(force_array) > 0:
        ...
        ax.plot(
          time_array, force_array, label=label, color=color, linewidth=1.5, alpha=0.8
        )
        color_idx += 1
    ...
    if body_data:
      ...
      if len(all_forces) > 0:
        ...
        print(
          f"[DEBUG] {title}: min={min_force:.2f}N, max={max_force:.2f}N, "
          f"links={len(body_data)}"
        )

  axes[-1].set_xlabel("Simulation Time (s)")
  fig.suptitle(
    "Contact Force Time Series (Max Force per Link)", fontsize=14, fontweight="bold"
  )
  fig.tight_layout()
...
def record_contact_forces(
  ...
  for _body_name, group in body_to_group.items():
    group_counts[group] += 1
  print(f"[INFO] Body groups: {dict(group_counts)}")

  # Store time series: group -> body_name -> (time_array, force_max_array)
  group_time_series: dict[str, dict[str, tuple[list[float], list[float]]]] = (
