...
from mjlab.utils.lab_api.rl.exporter import (
  export_policy_as_onnx as mjlab_export_policy_as_onnx,
)
...
class MjlabAmpOnPolicyRunner:
        ...
        for _ in range(self.num_steps_per_env):
          ...
          for key, value in obs.items():
            if torch.is_tensor(value) and torch.is_floating_point(value):
              obs[key] = torch.nan_to_num(value, nan=0.0, posinf=0.0, neginf=0.0)
          ...
          task_weight_scale = float(getattr(self.env, "task_reward_weight_scale", 1.0))
  ...
  def log(self, locs: dict, width: int = 80, pad: int = 35):
    ...
    writer.add_scalar("Loss/grad_pen_loss", locs["mean_grad_pen_loss"], locs["it"])
    ...
    writer.add_scalar("Loss/accuracy_policy", locs["mean_accuracy_policy"], locs["it"])
    writer.add_scalar("Loss/accuracy_expert", locs["mean_accuracy_expert"], locs["it"])
    writer.add_scalar("Loss/learning_rate", self.alg.learning_rate, locs["it"])
    writer.add_scalar("Loss/mean_kl_divergence", locs["mean_kl_divergence"], locs["it"])
    ...
    writer.add_scalar("Debug/actor_grad_norm", locs["mean_actor_grad_norm"], locs["it"])
    writer.add_scalar(
    ...
    writer.add_scalar("Policy/mean_noise_std", mean_std_value.item(), locs["it"])
    ...
    global_timesteps = (locs["it"] + 1) * self.num_steps_per_env * self.env.num_envs
