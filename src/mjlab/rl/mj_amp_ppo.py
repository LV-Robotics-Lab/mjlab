...
class _CombinedOptimizer:
  ...
  def __init__(
    self, policy_optimizer: optim.Optimizer, disc_optimizer: optim.Optimizer
  ):
...
class MjlabAmpPPO:
  ...
  def __init__(
    ...
    self.amp_storage = ReplayBuffer(
      obs_dim=obs_dim, buffer_size=buffer_size, device=device
    )
  ...
  def _update_discriminator(
    ...
    for _ in range(num_updates):
      ...
      if self.disc_input_noise_std > 0.0:
        policy_pair = policy_pair + self.disc_input_noise_std * torch.randn_like(
          policy_pair
        )
        expert_pair = expert_pair + self.disc_input_noise_std * torch.randn_like(
          expert_pair
        )
